#ifndef CLICK_MYIPREWRITER_HH
#define CLICK_MYIPREWRITER_HH
...
MyIPRewriter(INPUTSPEC1, ..., INPUTSPECn [, I<keywords>])
...
MyIPRewriter implements the functionality of a network address/port translator
...
MyIPRewriter maintains a I<mapping table> that records how packets are
...
When MyIPRewriter receives a packet, it first looks up that packet in the
...
address and port unchanged.  SPORT may be a port range 'L-H'; MyIPRewriter will
...
MyIPRewriter has no mappings when first initialized.
...
on a 'pass' input port.  MyIPRewriter changes IP packet data and, optionally,
...
dropped if an MyIPRewriter's mapping table is full of guaranteed flows.
...
Returns the number of mappings in this MyIPRewriter's tables.

=h mapping_failures r

Returns the number of mapping failures, which can occur, for example, when the
MyIPRewriter runs out of source ports, or when a new flow is dropped because the
MyIPRewriter is full.
...
Returns a human-readable description of the MyIPRewriter's current TCP mapping
table. An unparsed mapping includes both directions' output ports; the
relevant output port is starred.

=h udp_table read-only

Returns a human-readable description of the MyIPRewriter's current UDP mapping
table.
...
=a TCPRewriter, IPAddrRewriter, IPAddrPairRewriter, MyIPRewriterPatterns,
RoundRobinIPMapper, FTPPortMapper, ICMPRewriter, ICMPPingRewriter */


class MyIPRewriterEntry {
public:
    IPFlowID orig_flow;
    IPFlowID changed_flow;
    int port;
};


class MyRewriterPattern {
public:
    IPAddress _saddr;
    ...
    MyRewriterPattern(): _saddr(), _sport(0), _daddr(), _dport(0), _variation_top(0), _sequential(false) {}
};
...
public:
    ...
    int rewrite_flowid(const IPFlowID &flowid,
                       IPFlowID &rewritten_flowid,
                       WritablePacket *p, HashMap<IPFlowID, MyIPRewriterEntry> *map);
};


class MyIPRewriter : public TCPRewriter { public:

    typedef UDPRewriter::UDPFlow UDPFlow;

    MyIPRewriter() CLICK_COLD;
    ~MyIPRewriter() CLICK_COLD;

    const char *class_name() const		{ return "MyIPRewriter"; }
  ...
  private:
    ...
    Vector<MyInputSpec> _my_input_specs;

    HashMap<IPFlowID, MyIPRewriterEntry> _map_udp;
    HashMap<IPFlowID, MyIPRewriterEntry> _map_tcp;
    ...
    void tcp_flow_apply(const MyIPRewriterEntry &e, WritablePacket *p);
    void udp_flow_apply(const MyIPRewriterEntry &e, WritablePacket *p);
    
    static inline Map &reply_udp_map(IPRewriterInput *rwinput) {
	MyIPRewriter *x = static_cast<MyIPRewriter *>(rwinput->reply_element);
	return x->_udp_map;
    }
...
inline void
MyIPRewriter::destroy_flow(IPRewriterFlow *flow)
{
    if (flow->ip_p() == IP_PROTO_TCP)
